...
set search_path = public
as $$
declare kid uuid; tid uuid; rec public.support_messages;
begin
  select id into kid from public.access_keys
  where upper(key)=upper(trim(_key)) and revoked=false limit 1;
  if kid is null then raise exception 'invalid_key'; end if;
  if coalesce(trim(_body),'') = '' then raise exception 'invalid_body'; end if;

  select t.id into tid from public.support_threads t where t.key_id = kid;
  if tid is null then
    insert into public.support_threads(key_id) values (kid) returning id into tid;
  end if;
  insert into public.support_messages(thread_id,sender_type,body)
  values(tid,'user',left(trim(_body),1000)) returning * into rec;
...
begin
  ...
  if coalesce(trim(_body),'') = '' then raise exception 'invalid_body'; end if;
  insert into public.support_messages(thread_id,sender_type,body)
  values(_thread_id,'admin',left(trim(_body),1000)) returning * into rec;
